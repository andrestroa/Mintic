...
def main():
    ...
    for id_ in ids:
        ...
        for i in range(len(varea)):  # Aqui el ciclo debe ir hasta la cantidad de elementos del vector varea
            ...
            if type_new == 'a':
                acum_type_ant[type_new] += max(0, ceil((area - area_old * ant_old) / 35600 ))  # Aqui debes realizar el calculo de las antenas de tipo a con su rango respectivo
            elif type_new == 'b':  # Aqui debes verificar que el tipo de antena nueva sea 'b'
                acum_type_ant[type_new] += max(0, ceil((area - area_old * ant_old) / 6800))
            elif type_new == 'c':
                acum_type_ant[type_new] += max(0, ceil((area - area_old * ant_old) / 59300 ))  # Aqui debes realizar el calculo de las antenas de tipo a con su rango respectivo
            elif type_new == 'd' :  # Aqui debes verificar que el tipo de antena nueva sea 'd'
                acum_type_ant[type_new] += max(0, ceil((area - area_old * ant_old) / 24200 ))  # Aqui debes realizar el calculo de las antenas de tipo a con su rango respectivo
            elif type_new == 'e':  # Aqui debes verificar que el tipo de antena nueva sea 'e'
                acum_type_ant[type_new] += max(0, ceil((area - area_old * ant_old) / 7400 ))  # Aqui debes realizar el calculo de las antenas de tipo a con su rango respectivo
