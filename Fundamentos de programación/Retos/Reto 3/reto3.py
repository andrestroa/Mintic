...
def departament_land():
    #Pide departamento y terreno verifica que el dep no sea -1 y lo regresa
...
def antenna_verification(data_collection,lands):
    #verifica si el terreno ingresado esta en el rango y si la antena existe si no se agrega 0
...
def antenna_count(data_collection):
    #Verifica si el dato es correcto si no agrega el resultado de la operación
...
def total_antennas(data_collection):
    #Asigna un nuevo dict con los valores importantes
...
def verificacion_of_values(clean_data):
    #organiza diccionarios con el departamento, total de antenas a instalar y el % de antenas tipo A
    #Verifica min y max
...
def complete_information(full_data,min_value,max_value):
    #Ordena la información y la imprime en pantalla    
